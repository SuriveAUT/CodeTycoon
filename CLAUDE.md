...
- `src/store/gameState.js` — single Solid.js `createStore` with all game state, save/load/normalize/migration (`VERSION` 9), cost helpers (`calcBuildingCost`, `maxAffordable`), unlock checks (`isTechUnlocked` also checks the tech's funding round via `techChapter`)
...
- Calibration: `--goals a,b,…` overrides the XP goals of rounds 0, 1, … in memory, `--hold N` keeps round N from completing so the XP rate inside it can be measured. The XP goals in chapters.js were set this way (~4 days per round).
